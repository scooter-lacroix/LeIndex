...
use crate::tiered::{TieredHnswConfig, TieredHnswIndex};
...
/// Vector index implementation
///
/// Enum wrapping brute-force, HNSW, or tiered HNSW+Turso vector backends.
...
pub enum VectorIndexImpl {
    ...
    /// Tiered HNSW (hot) + Turso (cold) index
    TieredHNSW(Box<TieredHnswIndex>),
}
...
impl VectorIndexImpl {
    ...
    pub fn is_empty(&self) -> bool {
        match self {
            ...
            Self::TieredHNSW(idx) => idx.is_empty(),
        }
    }
    ...
    pub fn dimension(&self) -> usize {
        match self {
            ...
            Self::TieredHNSW(idx) => idx.dimension(),
        }
    }
    ...
    pub fn search(&self, query: &[f32], top_k: usize) -> Vec<(String, f32)> {
        match self {
            ...
            Self::TieredHNSW(idx) => idx.search(query, top_k),
        }
    }
...
matches!(self, Self::HNSW(_) | Self::TieredHNSW(_))
    }
}
...
///
/// Supports brute-force, HNSW, and tiered HNSW+Turso vector search backends.
///
/// # Thread Safety
///
...
impl SearchEngine {
    ...
    /// Create engine with HNSW + Turso tiered index
    ///
    /// Creates a new search engine with HNSW-based hot tier and Turso-backed cold tier.
    /// HNSW provides fast ANN for hot vectors; cold vectors spill to Turso when
    /// the hot-tier memory budget is reached.
    ///
    /// # Arguments
    ///
    ...
    pub fn with_hnsw(dimension: usize, hnsw_params: HNSWParams) -> Self {
        Self::with_hnsw_turso(dimension, hnsw_params, TieredHnswConfig::default())
    }

    /// Create engine with HNSW + Turso tiered storage using explicit config.
    ///
    /// Remote Turso is optional. By default use `TursoConfig::local_only()` to keep
    /// plug-and-play local behavior.
    #[must_use]
    pub fn with_hnsw_turso(
        dimension: usize,
        hnsw_params: HNSWParams,
        tiered_config: TieredHnswConfig,
    ) -> Self {
        // Validate dimension at construction time
        if !(MIN_EMBEDDING_DIMENSION..=MAX_EMBEDDING_DIMENSION).contains(&dimension) {
            panic!(
        ...
        Self {
            nodes: Vec::new(),
            scorer: HybridScorer::new(),
            vector_index: VectorIndexImpl::TieredHNSW(Box::new(TieredHnswIndex::new(
                dimension,
                hnsw_params,
                tiered_config,
            ))),
    ...
    pub fn enable_hnsw(&mut self, params: HNSWParams) -> Result<(), Error> {
        ...
        let mut new_tiered_index = TieredHnswIndex::new(dimension, params, TieredHnswConfig::default());
        ...
        for node in &self.nodes {
            if let Some(ref embedding) = node.embedding {
                match new_tiered_index.insert(node.node_id.clone(), embedding.clone()) {
                    Ok(()) => migrated += 1,
                    Err(e) => {
        ...
        tracing::info!(
            "Migrated to tiered HNSW/Turso index: {}/{} embeddings transferred, {} failed",
            migrated,
            total_embeddings,
            failed
        );
        ...
        self.vector_index = VectorIndexImpl::TieredHNSW(Box::new(new_tiered_index));
        Ok(())
    }
