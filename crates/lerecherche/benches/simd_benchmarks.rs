...
use lerecherche::quantization::{Quantize, Int8QuantizedVector};
use lerecherche::quantization::simd::{dot_product_adc, fallback};
...
fn benchmark_throughput(c: &mut Criterion) {
    ...
    let quantized_vectors: Vec<_> = (0..batch_size)
        .map(|_| quantized.clone())
        .collect();
...
fn benchmark_comparison(c: &mut Criterion) {
    ...
    group.bench_function("auto_768d", |b| {
        b.iter(|| dot_product_adc(black_box(&query), black_box(&quantized), black_box(query_sum)));
    });
    
    group.bench_function("fallback_768d", |b| {
        b.iter(|| fallback::dot_product_adc(black_box(&query), black_box(&quantized), black_box(query_sum)));
    });
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            ...
            group.bench_function("avx2_768d", |b| {
                b.iter(|| unsafe {
                    x86_avx2::dot_product_adc(black_box(&query), black_box(&quantized), black_box(query_sum))
                });
            });
        }
    }
