...
impl CodeIntelligence for RubyParser {
    ...
    fn get_signatures_with_parser(
        ...
        parser
            .set_language(&crate::parse::traits::languages::ruby::language())
...
fn extract_ruby_parameters(node: &tree_sitter::Node<'_>, source: &[u8]) -> Vec<Parameter> {
...
mod tests {
  ...
  def initialize(name)
    ...
    #[test]
    fn test_ruby_complexity_calculation() {
        let source = b"def complex(x)
  if x > 0
    (0...x).each do |i|
      if i.even?
        puts i
        ...
        let metrics = ruby_parser.extract_complexity(&root);
