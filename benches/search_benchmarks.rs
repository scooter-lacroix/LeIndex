...
fn benchmark_search_latency(c: &mut Criterion) {
    ...
    for size in &index_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("int8_search", size), size, |b, _| {
            b.iter(|| {
...
fn benchmark_index_building(c: &mut Criterion) {
    ...
    for size in &index_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("build_int8_index", size), size, |b, _| {
            b.iter_with_setup(
                || vectors.clone(),
                |vecs| {
                    let params = Int8HnswParams::new().with_m(16).with_ef_construction(200);
                    let mut index = Int8HnswIndex::with_params(dim, params);
...
fn benchmark_memory_efficiency(c: &mut Criterion) {
    ...
    for size in &index_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("int8_memory", size), size, |b, _| {
            b.iter_with_setup(
                || vectors.clone(),
                |vecs| {
                    let params = Int8HnswParams::new();
