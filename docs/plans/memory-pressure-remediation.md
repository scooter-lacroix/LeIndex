...
> ✅ **DONE (2026-08-03)** — steps (1)-(2) verified + **T8a** (`f7fad073`): worker-binary resolution handles the merged `-p leindex --bin leindex-embed` layout; sentinels root-caused; budget gate de-flaked (embed_idle→8020, embed_teardown→7900, tolerance 5→10%). **Step-3 empirical baseline capture**: `cargo xtask memcheck --update-baseline` measured all 9 phases for real (idle_warm 8900 · idle_post 8912 · idle_final 8656 · embed_idle 8684 · embed_teardown 9104 · embed_active main 20240/worker 2594916/combined 2615156 KiB; re-gate **ALL PHASES PASSED ✓** · memcheck 10/0; `rebaselined_note` restored). **Step-3 new phases (commit `481f72e4`)**: canonical phases 9→12 — `mcp_idle_proliferation` (3 concurrent idle MCP servers, combined 24880 KiB = 2.8× single idle_warm 8908, linear; guard ≤3.5× in harness test) · `worker_ort_threads` (worker-active trigger pinned `LEINDEX_WORKER_ORT_THREADS=1`; worker 2540352/combined 2560544 KiB; {0,4,1} record t0 1497384 / t4 1474980 / t1 1492168 KiB, cap≤no-cap ✓ — model weights dominate, thread cap saves ~5–22 MiB) · `stale_artifacts` (seeds dead-pid sidecars, `leindex cleanup --stale-daemons` sweep 404 KiB, all seeded sidecars removed). Re-gate **ALL PHASES PASSED ✓** (baseline +5%, ceiling +10%) · `cargo test -p memcheck` 13/13 (bin 41 + harness 13) · clippy -D 0 · fmt clean. `rebaselined_note` restored on 9 baseline files.
