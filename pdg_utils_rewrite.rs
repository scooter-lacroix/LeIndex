...
pub struct RelinkConfig {
    ...
    /// Set to 1 for strict (one best match) or 2-3 for permissive (keep alternatives).
...
impl RelinkConfig {
    ...
    pub fn strict() -> Self {
        Self { max_candidates: 1, ..Self::default() }
    }
}
...
pub fn merged_pdg_from_results(
    ...
    for result in results {
        ...
        let language = result.language.as_ref()
...
fn merge_pdgs_with_keys(
    ...
    for edge_idx in source.edge_indices() {
        let Some(edge) = source.get_edge(edge_idx) else { continue };
        let Some((from, to)) = source.edge_endpoints(edge_idx) else { continue };
        let (Some(from_node), Some(to_node)) = (
            source.get_node(from), source.get_node(to),
        ) else { continue };
...
pub fn relink_external_import_edges(
    ...
    for node_idx in pdg.node_indices() {
        ...
        if node.node_type == NodeType::Module && node.language == "external" { continue; }
    ...
    for edge_idx in edge_indices {
        let Some(edge) = pdg.get_edge(edge_idx) else { continue };
        if edge.edge_type != EdgeType::Import { continue; }
        let Some((from, to)) = pdg.edge_endpoints(edge_idx) else { continue };
        let Some(target) = pdg.get_node(to) else { continue };
        if !(target.node_type == NodeType::Module && target.language == "external") { continue; }

        let importer_lang = pdg.get_node(from)
            .map(|n| n.language.clone())
            .unwrap_or_default();

        let candidates: Vec<NodeId> = resolve_import_candidates_ranked(
            &target.name, &symbol_map, pdg, &importer_lang, config,
        )
        .into_iter()
        .filter(|c| *c != from)
    ...
    let orphans: Vec<NodeId> = external_degree.keys().copied()
        .filter(|&nid| {
            let Some(node) = pdg.get_node(nid) else { return false };
            node.node_type == NodeType::Module && node.language == "external"
...
fn resolve_import_candidates_ranked(
    ...
    if let Some(exact) = symbol_map.get(&normalized) {
        for id in exact { *scored.entry(*id).or_insert(0) += config.exact_match_score; }
    }
    ...
    for len in 2..=3_usize.min(parts.len()) {
        ...
        if let Some(values) = symbol_map.get(&key) {
            for id in values { *scored.entry(*id).or_insert(0) += config.suffix_match_score; }
        }
    }
    ...
    if let Some(last) = normalized.split('.').last() {
        if let Some(values) = symbol_map.get(last) {
            for id in values { *scored.entry(*id).or_insert(0) += config.last_seg_score; }
        }
    }
    ...
    ranked.sort_by(|(la, ls), (ra, rs)| {
        ...
        (rs + rb).cmp(&(ls + lb)).then_with(|| la.index().cmp(&ra.index()))
    });
    ...
    if looks_module_like {
        let module_ids: Vec<NodeId> = ranked_ids.iter().copied()
            .filter(|id| pdg.get_node(*id).map(|n| n.node_type == NodeType::Module).unwrap_or(false))
            .collect();
        if !module_ids.is_empty() { return module_ids; }
    }
...
fn node_bonus(
    ...
    let Some(node) = pdg.get_node(node_id) else { return 0 };
    let mut bonus = 0;
    if node.language == importer_language { bonus += config.same_language_bonus; }
    if looks_module_like && node.node_type == NodeType::Module { bonus += config.module_node_bonus; }
    bonus
}
...
fn candidate_keys_for_node(node: &crate::graph::pdg::Node) -> Vec<String> {
    ...
    let norm_id = node.id.split_once(':')
    ...
    if !norm_name.is_empty() { keys.insert(norm_name); }
...
mod tests {
    ...
    fn make_sig(name: &str, qualified: &str, imports: Vec<ImportInfo>) -> SignatureInfo {
        SignatureInfo {
            ...
            docstring: None, calls: Vec::new(), imports, byte_range: (0, 10),
        }
    }

    fn make_result(path: &str, language: &str, sigs: Vec<SignatureInfo>, src: &[u8]) -> ParsingResult {
        ParsingResult {
    ...
    fn relinks_external_to_internal() {
        let a = make_result("src/a.rs", "rust", vec![
            make_sig("main", "pkg::main", vec![ImportInfo { path: "pkg.helper".to_string(), alias: None }]),
        ], b"");
        let b = make_result("src/b.rs", "rust", vec![
            make_sig("helper", "pkg::helper", vec![]),
        ], b"");

        let pdg = merged_pdg_from_results(&[a, b], None);

        let external_count = pdg.edge_indices()
            .filter_map(|idx| {
                let edge = pdg.get_edge(idx)?;
                if edge.edge_type != EdgeType::Import { return None; }
                let (_, to) = pdg.edge_endpoints(idx)?;
                let target = pdg.get_node(to)?;
                (target.node_type == NodeType::Module && target.language == "external").then_some(())
            })
            .count();
    ...
    fn strict_config_keeps_single_candidate() {
        let a = make_result("src/a.rs", "rust", vec![
            make_sig("main", "pkg::main", vec![ImportInfo { path: "util".to_string(), alias: None }]),
        ], b"");
        let b = make_result("src/b.rs", "rust", vec![make_sig("util", "alpha::util", vec![])], b"");
        let c = make_result("src/c.rs", "rust", vec![make_sig("util", "beta::util", vec![])], b"");
    ...
    fn permissive_config_allows_multiple_candidates() {
        ...
        let config = RelinkConfig { max_candidates: 3, ..RelinkConfig::default() };
        let pdg = merged_pdg_from_results(&[a, b, c], Some(config));
        let import_count = pdg.edge_indices()
            .filter_map(|idx| pdg.get_edge(idx))
            .filter(|e| e.edge_type == EdgeType::Import)
            .count();
        assert!(import_count >= 2, "Permissive mode: should relink to both candidates");
    }

    #[test]
    fn containment_edges_preserved_through_merge() {
        let a = make_result("src/a.rs", "rust", vec![
            { let mut s = make_sig("method", "MyClass::method", vec![]); s.is_method = true; s }
        ], b"");

        let pdg = merged_pdg_from_results(&[a], None);
        let containment = pdg.edge_indices()
            .filter_map(|idx| pdg.get_edge(idx))
