...
print_header() {
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC} ${GREEN}LeIndex Installer v2.0.0 for macOS${NC} ${CYAN}                    ║${NC}"
...
verify_macos() {
    if [[ "$OSTYPE" != "darwin"* ]]; then
        ...
        echo "Please use install.sh for Linux/Unix systems."
        exit 1
    fi
...
check_pip() {
    ...
    if command -v pip3 &> /dev/null; then
        ...
        echo "  If using Homebrew Python: pip should be already installed"
        echo "  Otherwise: easy_install pip"
        exit 1
    fi
    echo ""
}
...
install_leindex() {
    ...
    if $PIP_CMD install leindex; then
        ...
        if command -v leindex &> /dev/null; then
            ...
            echo "  You may need to: export PATH=\"\$PATH:\$HOME/Library/Python/3.*/bin\""
...
configure_vscode() {
    ...
    # Check for VS Code in Applications
    if [ -d "/Applications/Visual Studio Code.app" ] || [ -d "/Applications/VS Code.app" ]; then
        print_success "VS Code detected in /Applications"
    else
        print_warning "VS Code not found in /Applications"
...
configure_zed() {
    ...
    # Check for Zed in Applications
    if [ -d "/Applications/Zed.app" ]; then
        print_success "Zed detected in /Applications"
    else
        print_warning "Zed not found in /Applications"
...
configure_cli_tools() {
    ...
    if command -v leindex-search &> /dev/null; then
        ...
        echo "  Add to PATH: export PATH=\"\$PATH:\$(python3 -m site --user-base)/bin\""
        echo ""
    fi
}
...
verify_installation() {
    ...
    if [ -f "$HOME/Library/Application Support/Code/User/settings.json" ] || [ -f "$HOME/.vscode/settings.json" ]; then
        if grep -q "leindex" "$HOME/Library/Application Support/Code/User/settings.json" 2>/dev/null || \
...
print_completion() {
    ...
    echo "  - If 'leindex' command is not in PATH, add to ~/.zshrc:"
    echo "    export PATH=\"\$PATH:\$(python3 -m site --user-base)/bin\""
...
rollback() {
    ...
    find "$HOME/.config" "$HOME/Library/Application Support" -name "*.backup.*" -type f 2>/dev/null | while read -r backup; do
