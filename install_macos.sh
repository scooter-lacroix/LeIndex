...
echo "Platform: macOS" >> "$INSTALL_LOG"
...
handle_error() {
    ...
    if [[ $exit_code -eq 1 ]]; then
        ...
        print_bullet "DEBUG=1 ./install_macos.sh"
    fi
}
...
detect_python() {
    ...
    if [[ -z "$PYTHON_CMD" ]]; then
        ...
        print_bullet "macOS (Homebrew): ${CYAN}brew install python@3.13${NC}"
        print_bullet "From installer: ${CYAN}https://www.python.org/downloads/${NC}"
        print_bullet "Or using pyenv: ${CYAN}brew install pyenv && pyenv install 3.13${NC}"
        exit 1
    fi
...
detect_ai_tools() {
    ...
    # Common binary locations for explicit checking (macOS-specific)
    local common_bins=("/usr/local/bin" "/usr/bin" "$HOME/.local/bin" "$HOME/bin" "/opt/homebrew/bin" "/opt/homebrew/sbin" "/opt/bin" "/usr/local/opt")
    ...
    # ============================================================================
    # EDITORS / IDEs - Check config directories (macOS paths)
    # ============================================================================

    # Claude Desktop (macOS uses Application Support)
    [[ -d "$HOME/Library/Application Support/Claude" ]] && detected_editors+=("claude-desktop")

    # Cursor IDE
    [[ -d "$HOME/.cursor" || -d "$HOME/.config/cursor" ]] && detected_editors+=("cursor")

    # Antigravity
    [[ -d "$HOME/.config/antigravity" || -d "$HOME/.antigravity" || -d "$HOME/Library/Application Support/Antigravity" ]] && detected_editors+=("antigravity")

    # VS Code / VSCodium (check both macOS and Linux-style config locations)
    [[ -d "$HOME/Library/Application Support/Code" || -d "$HOME/.config/Code" ]] && detected_editors+=("vscode")
    ...
    # JetBrains IDEs
    [[ -d "$HOME/Library/Application Support/JetBrains" || -d "$HOME/.config/JetBrains" ]] && detected_editors+=("jetbrains")
...
# Configure Claude Desktop (macOS path)
configure_claude_desktop() {
    ...
    # If no existing config found, create in macOS default location
...
configure_vscode() {
    ...
    for config_file in "${vscode_configs[@]}"; do
        ...
        if [[ -d "$(dirname "$config_dir")" ]] || [[ "$config_file" == *".vscode"* ]]; then
            mkdir -p "$config_dir"
            backup_file "$config_file" 2>/dev/null || true
...
configure_jetbrains() {
    ...
    local config_dir="$HOME/Library/Application Support/JetBrains"
...
PYTHON_EOF
    then
        ...
        print_warning "Failed to configure Antigravity"
        return 2
    fi
}
...
PYTHON_EOF
    then
        ...
        print_warning "Failed to configure Droid CLI"
        return 2
    fi
}
...
select_tools() {
    ...
    case $choice in
        ...
        18)
            ...
            [[ -d "$HOME/Library/Application Support/JetBrains" ]] && configure_jetbrains
...
verify_installation() {
    ...
    if [[ -f "$HOME/Library/Application Support/Code/User/settings.json" ]] && grep -q "leindex" "$HOME/Library/Application Support/Code/User/settings.json" 2>/dev/null; then
...
print_completion() {
    ...
    print_bullet "To see all debug info, run: ${CYAN}DEBUG=1 ./install_macos.sh${NC}"
    echo ""
