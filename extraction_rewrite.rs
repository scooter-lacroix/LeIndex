...
use crate::graph::pdg::{
    Edge, EdgeMetadata, EdgeType, EmbeddingStore, Node, NodeType, ProgramDependenceGraph,
};
...
pub fn extract_pdg_from_signatures(
    ...
    let containment = infer_class_nodes_and_containment(
        &signatures, &mut pdg, &mut node_ids, file_path, language,
    );
    ...
    let import_edges = extract_import_edges(
        &signatures, &node_ids, &mut pdg, file_path, language, source_code,
    );
...
fn infer_class_nodes_and_containment(
    ...
    for (class_name, method_nids) in &class_methods {
        ...
        if already_exists {
            ...
            if let Some(&class_nid) = node_ids.get(class_name)
                .or_else(|| node_ids.iter().find(|(k, _)| normalize_symbol(k) == *class_name).map(|(_, v)| v))
            {
        ...
        let short_name = class_name.rsplit('.').next().unwrap_or(class_name).to_string();
        let class_node = Node {
            id: format!("{}:{}", file_path, class_name),
            node_type: NodeType::Class,
            name: short_name,
            file_path: file_path.to_string(),
            byte_range: (if min_start == usize::MAX { 0 } else { min_start }, max_end),
...
const EXCLUDED_TYPES: &[&str] = &[
    "str", "string", "String", "&str", "int", "i32", "i64", "u32", "u64", "usize",
    "f32", "f64", "bool", "void", "None", "null", "undefined", "any", "Any",
    "object", "Object", "self", "Self", "cls", "this", "bytes", "Bytes",
    "Vec", "List", "list", "dict", "Dict", "HashMap", "Option", "Result",
    "Error", "Exception", "T", "U", "K", "V",
];
...
pub fn extract_data_flow_edges(
...
) -> Vec<(crate::graph::pdg::NodeId, crate::graph::pdg::NodeId, String, f32)> {
    ...
    for sig in signatures {
        ...
        let calls: HashSet<String> = sig.calls.iter()
            .map(|c| normalize_symbol(c))
            .collect();
        ...
        if let Some(consumer_sigs) = consumers.get(type_name) {
            for prod in producer_sigs {
                for cons in consumer_sigs {
                    if prod.qualified_name == cons.qualified_name { continue; }
    ...
    for (type_name, ret_sigs) in &producers {
        ...
        for i in 0..ret_sigs.len() {
            for j in 0..ret_sigs.len() {
                ...
                let a_calls_b = call_set.get(&a_norm).map(|s| s.contains(&b_norm)).unwrap_or(false);
                if a_calls_b {
                    let (Some(&from), Some(&to)) = (
    ...
    for sig_a in signatures {
        ...
        let Some(a_calls) = call_set.get(&a_norm) else { continue };
        for called_norm in a_calls {
            let Some(callee_sigs) = by_normalized_name.get(called_norm) else { continue };
            for sig_b in callee_sigs {
                // Find shared param types
                let a_types: HashSet<String> = sig_a.parameters.iter()
                    .filter_map(|p| p.type_annotation.as_ref())
                    .map(|t| normalize_type_name(t))
                    .filter(|t| !t.is_empty() && !is_excluded_type(t))
                    .collect();
                let b_types: HashSet<String> = sig_b.parameters.iter()
                    .filter_map(|p| p.type_annotation.as_ref())
                    .map(|t| normalize_type_name(t))
                    .filter(|t| !t.is_empty() && !is_excluded_type(t))
                    .collect();
...
const COMMON_METHOD_NAMES: &[&str] = &[
    "new", "init", "__init__", "constructor", "create", "build",
    "toString", "to_string", "__str__", "__repr__", "equals", "__eq__",
    "hashCode", "__hash__", "clone", "__clone__", "copy", "dispose",
    "close", "__del__", "finalize", "update", "get", "set",
...
const ABSTRACT_BASE_SUFFIXES: &[&str] = &[
    "Base", "Mixin", "Interface", "Protocol", "Trait", "ABC", "Abstract",
];

fn looks_like_abstract_base(class_name: &str) -> bool {
    ABSTRACT_BASE_PREFIXES.iter().any(|p| class_name.starts_with(p) && class_name.len() > p.len())
        || ABSTRACT_BASE_SUFFIXES.iter().any(|s| class_name.ends_with(s) && class_name.len() > s.len())
}
...
pub fn extract_inheritance_edges(
    ...
    if class_names.len() < 2 { return edges; }
        ...
        for j in 0..class_names.len() {
            ...
            for method_a in methods_a {
                ...
                let calls_super = method_a.calls.iter().any(|call| {
                    ...
                    super_patterns.iter().any(|pat| norm_call.ends_with(&normalize_symbol(pat)))
            ...
            let shared_count = names_a.intersection(&names_b)
            ...
            let (child_cls, parent_cls) = determine_inheritance_direction(
                cls_a, cls_b, methods_a, methods_b,
                &evidence, short_a, short_b,
            );
            ...
            let child_nid = node_ids.get(child_cls)
                .or_else(|| class_methods.get(child_cls)
                    .and_then(|m| m.first())
                    .and_then(|sig| node_ids.get(&sig.qualified_name)))
                .copied();
            let parent_nid = node_ids.get(parent_cls)
                .or_else(|| class_methods.get(parent_cls)
                    .and_then(|m| m.first())
                    .and_then(|sig| node_ids.get(&sig.qualified_name)))
                .copied();
    ...
    if evidence.super_call_confidence > 0.0 {
        let a_calls_super = methods_a.iter().any(|sig| {
            sig.calls.iter().any(|c| {
                ...
                norm.starts_with("super.") || norm.starts_with("super::")
                    || norm.starts_with("parent.") || norm.contains(cls_b)
            })
        });
        if a_calls_super { return (cls_a, cls_b); }
        return (cls_b, cls_a);
    }

    // Naming convention: abstract base is the parent
    if looks_like_abstract_base(short_a) { return (cls_b, cls_a); }
    if looks_like_abstract_base(short_b) { return (cls_a, cls_b); }

    // Qualified name nesting: more nested class is the child
    if cls_b.starts_with(cls_a) { return (cls_b, cls_a); }
    if cls_a.starts_with(cls_b) { return (cls_a, cls_b); }

    // Fallback: shorter name = parent (less specific = more abstract)
    if cls_a.len() <= cls_b.len() { (cls_b, cls_a) } else { (cls_a, cls_b) }
}
    ...
    for sig in signatures {
        for import in &sig.imports {
            let alias = import.alias.clone().or_else(|| {
                import.path.split(|c| c == '.' || c == ':' || c == '/' || c == '\\')
                    .last().map(|s| s.to_string())
            });
            if let Some(alias) = alias {
                alias_map.entry(alias).or_insert_with(|| import.path.clone());
            }
        }
    }

    for sig in signatures {
        let Some(&caller_id) = node_ids.get(&sig.qualified_name) else { continue };
        let caller_ns = {
            let norm = normalize_symbol(&sig.qualified_name);
            let segs: Vec<&str> = norm.split('.').collect();
            if segs.len() > 1 { Some(segs[..segs.len()-1].join(".")) } else { None }
        };
        ...
        for call_target in &sig.calls {
            ...
            if let Some(ns) = &caller_ns {
                if let Some(first) = call_segs.first() {
                    if matches!(first.as_str(), "self" | "this" | "super" | "Self" | "crate" | "base") {
            ...
            for candidate in candidates {
                ...
                if let Some(ids) = exact_map.get(&norm) { targets.extend(ids); }
                if let Some(last) = segs.last() {
                    if let Some(ids) = last_map.get(*last) { targets.extend(ids); }
                }
                for len in 2..=3_usize.min(segs.len()) {
                    let start = segs.len() - len;
                    let key = segs[start..].join(".");
                    if let Some(ids) = suffix_map.get(&key) { targets.extend(ids); }
                }
            }
...
pub fn extract_import_paths_from_source(source_code: &[u8], language: &str) -> HashSet<String> {
    let Ok(source) = std::str::from_utf8(source_code) else { return HashSet::new(); };
...
fn extract_python_imports(source: &str) -> HashSet<String> {
    ...
    for cap in re_import.captures_iter(source) {
        for name in cap[1].split(',') {
            ...
            if !trimmed.is_empty() { imports.insert(trimmed.to_string()); }
        }
    }

    // `from x import (...)` — multi-line via DOTALL
    // First capture the module name, then the import list
    let re_from = Regex::new(r"(?s)from\s+([\w.]+)\s+import\s+(?:\(([^)]+)\)|(\w[\w\s,*]*))").unwrap();
    for cap in re_from.captures_iter(source) {
        ...
        for name in names_str.split(',') {
            let sym = name.split_whitespace().next().unwrap_or("").trim().trim_matches('*');
            if !sym.is_empty() && sym != "*" {
...
fn extract_js_ts_imports(source: &str) -> HashSet<String> {
    ...
    let re_named = Regex::new(r#"(?s)import\s+(?:type\s+)?\{[^}]*\}\s+from\s+['"]([^'"]+)['"]"#).unwrap();
    for cap in re_named.captures_iter(source) {
        imports.insert(cap[1].trim().to_string());
    }

    // import x from 'y'  / import * as x from 'y'
    let re_default = Regex::new(r#"import\s+(?:type\s+)?(?:\*\s+as\s+\w+|\w+)\s+from\s+['"]([^'"]+)['"]"#).unwrap();
...
fn expand_rust_use(stmt: &str, out: &mut HashSet<String>) {
    ...
    if let Some(brace_start) = stmt.find('{') {
        let base = stmt[..brace_start].trim().trim_end_matches("::").replace("::", ".");
        let inner = stmt[brace_start + 1..].trim_end_matches('}').trim_end_matches(';');
        // Recursively handle nested braces
        for item in split_respecting_braces(inner) {
            let item = item.trim();
            if item == "self" { out.insert(base.clone()); continue; }
            if item.contains('{') {
...
fn collapse_multiline(source: &str, prefix: &str, terminator: &str) -> Vec<String> {
    ...
    for line in source.lines() {
        ...
        if in_stmt && current.contains(terminator) {
            let end = current.find(terminator).unwrap();
...
fn extract_php_imports(source: &str) -> HashSet<String> {
    ...
    // require/include
    let re2 = Regex::new(r#"(?:require|include)(?:_once)?\s*\(?['"]([^'"]+)['"]\)?"#).unwrap();
...
fn extract_scala_imports(source: &str) -> HashSet<String> {
    let mut imports = HashSet::new();
    // import x.y.{A, B, C} — multi-line
    let collapsed = collapse_multiline(source, "import ", "\n");
    for stmt in &collapsed {
        let re = Regex::new(r"import\s+([\w.]+)(?:\.\{([^}]+)\}|\.(\w+|\*))?").unwrap();
        if let Some(cap) = re.captures(stmt) {
            let base = &cap[1];
            if let Some(names) = cap.get(2) {
                for name in names.as_str().split(',') {
                    let n = name.trim();
                    if n != "_" { imports.insert(format!("{}.{}", base, n)); }
                }
...
fn extract_import_edges(
    ...
    let mut unique_paths: HashSet<String> = signatures.iter()
    ...
    if unique_paths.is_empty() { return edges; }
    ...
    for path in unique_paths {
        ...
        for target in targets {
            if target == importer_nid { continue; }
...
fn resolve_import_targets(
    ...
    if let Some(ids) = symbol_map.get(&normalized) { targets.extend(ids); }

    let parts: Vec<&str> = normalized.split('.').collect();
    for len in 2..=3_usize.min(parts.len()) {
        let start = parts.len() - len;
        let key = parts[start..].join(".");
        if let Some(ids) = symbol_map.get(&key) { targets.extend(ids); }
    }

    if targets.is_empty() {
        if let Some(last) = normalized.split('.').last() {
            if let Some(ids) = symbol_map.get(last) { targets.extend(ids); }
        }
    }
...
fn signature_to_node(sig: &SignatureInfo, file_path: &str, language: &str) -> Node {
    let node_type = if sig.is_method { NodeType::Method } else { NodeType::Function };
...
mod tests {
    ...
    fn sig(name: &str, qualified: &str, is_method: bool) -> SignatureInfo {
        SignatureInfo {
            name: name.to_string(), qualified_name: qualified.to_string(),
            parameters: vec![], return_type: None, visibility: Visibility::Public,
            is_async: false, is_method, docstring: None, calls: vec![],
            imports: vec![], byte_range: (0, 100),
        }
    }

    fn sig_with_types(name: &str, qualified: &str, params: Vec<(&str, &str)>, ret: Option<&str>) -> SignatureInfo {
        let parameters = params.into_iter().map(|(pname, ptype)| Parameter {
        ...
        SignatureInfo {
            name: name.to_string(), qualified_name: qualified.to_string(),
            parameters, return_type: ret.map(|s| s.to_string()),
            visibility: Visibility::Public, is_async: false, is_method: false,
            docstring: None, calls: vec![], imports: vec![], byte_range: (0, 100),
        }
    }
    ...
    fn containment_edges_are_not_call_edges() {
        ...
        assert_eq!(containment_count, 1, "Should have one Class→Method containment edge");
    }
    ...
    fn data_flow_signal_a_produces_directed_edge() {
        ...
        assert!(!edges.is_empty(), "Signal A should produce a data flow edge");
    ...
    fn data_flow_clique_not_generated() {
        ...
        let sigs: Vec<SignatureInfo> = (0..10).map(|i| {
            sig_with_types(&format!("f{i}"), &format!("f{i}"),
                vec![("s", "String")], None)
        }).collect();
        let mut nids = HashMap::new();
        let mut pdg = ProgramDependenceGraph::new();
        ...
        // Signal C same
        assert_eq!(edges.len(), 0, "Shared param type without call or return relationship should not produce edges");
    }
    ...
    fn inheritance_super_call_signal() {
        ...
        let inheritance_edges: Vec<_> = pdg.edge_indices()
            .filter_map(|e| {
        ...
        assert!(!inheritance_edges.is_empty(), "Super call should produce inheritance edge");
        assert!(inheritance_edges[0] >= 0.85, "Super call confidence should be high");
    }
